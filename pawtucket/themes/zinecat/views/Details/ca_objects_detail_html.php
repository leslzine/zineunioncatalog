...
//changed by eric to pull in text of a list instead of the id
									if($vs_value = $t_object->get("ca_objects.{$vs_attribute_code}",array('returnAsLink' => 0, 'hierarchicalDelimiter' => ' > ', 'convertCodesToDisplayText' => true))){
...
<?php
			...
			# --- entities
			$va_entities = $t_object->get("ca_entities", array("returnAsArray" => 1, 'checkAccess' => $va_access_values, 'sort' => 'display_name'));

                    
			if(sizeof($va_entities) > 0){	
?>
				<div class="unit"><h2><?php print _t("Created")." ".((sizeof($va_entities) > 1) ? _t("By") : _t("By")); ?></h2>
<?php
				foreach($va_entities as $va_entity) {
//					print "<div>".(($this->request->config->get('allow_detail_for_ca_entities')) ? caNavLink($this->request, $va_entity["display_name"], '', 'Detail', 'Entity', 'Show', array('entity_id' => $va_entity["entity_id"])) : $va_entity["label"])." (".$va_entity['relationship_typename'].")</div>";
				}

                        $va_entities = $t_object->get("ca_entities", array("template" => "<l>^ca_entities.preferred_labels</l> (^relationship_typename)", "returnAsLink" => true, "returnAllLocales" => false, "return\
AsArray" => true, 'checkAccess' => $va_access_values, 'sort' => 'forname'));
                        if(sizeof($va_entities) > 0){
                                print "<div class='unit'>";
//<h2>"._t("Related entities")."</h2>";
                                print $va_entities;
                                print "</div><!-- end unit -->";
                        }
...
$va_subjects = $t_object->get("ca_objects.subjects", array('returnAsArray' => true, 'returnAllLocales' => false));
//print_r($va_subjects);
if(sizeof($va_subjects) > 0){
print "<div class= 'unit'><h2>"._t("Keyword").((sizeof($va_subjects) > 1) ? "s" : "")." </h2>";
print "<div>";
foreach($va_subjects as $va_subjects) {
		 

// Regex to render the output searchable within CA
$string_for_output = caNavLink($this->request, $va_subjects['subjects'], '', '', 'Search', 'Index', array
('search' => 'ca_objects.keywords:'.trim($va_subjects['subjects']).''));
...
<?php
			...
			if(sizeof($va_places) > 0){
				print "<div class='unit'><h2>"._t("Place Created").((sizeof($va_places) > 1) ? "s" : "")."</h2>";
...
$va_datetype = $t_object->get("ca_objects.date", array('sort' => array('ca_objects.date.dates_value'), 'convertLinkBreaks'  => true, 'convertCodesToDisplayText' => true, 'template' => "^dates_value (^dc_dates_types)", 'delimiter' => '<br/>'));
				
 if($t_object->get("ca_objects.date")){
// get all dates in an array

    $dates_to_print = $t_object->get("ca_objects.date", array('returnAsArray' => 1, 'convertCodesToDisplayText' => true,));
      if(sizeof($va_datetype) > 0){

        $how_many_dates = count($dates_to_print);

// if there's only one date, print DATE as title
          if ($how_many_dates == 1){
            print "<div class='unit'><h2>"._t("Date")."</h2>";
// if there are 2 or more dates, print DATES as title
          }else{
            print "<div class='unit'><h2>"._t("Dates")."</h2>";
          }

//loop over items in array, printing each on a separate line
        foreach ($dates_to_print as $date_to_print) {
          print $date_to_print['dates_value']." (".$date_to_print['dc_dates_types'].")";
          print "<br>";
        }
        print "</div><!-- end unit -->";
   }}
			...
			if (sizeof($va_related_objects)) {
				print "<div class='unit'><h2>"._t("Related Zines")."</h2>";
